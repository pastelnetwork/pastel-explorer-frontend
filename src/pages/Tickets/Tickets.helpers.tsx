...
export const transformInferenceAPICreditPackData = (data: TicketsList[]) =>
  data.map(
    ({ transactionHash, timestamp, pastelID, image, reTxId, ticketType, contract_ticket }) => {
      const getInfo = () => {
        if (ticketType === 'sense') {
          return {
            link: `${ROUTES.SENSE_DETAILS}?txid=${reTxId}`,
        ...
        [TXID_KEY]: (
          <Grid container spacing={3}>
            ...
            <Grid item xs={12} sm={6} md={3}>
              ...
              <Box className="bold">
                {parseContractTicket?.ticket_input_data_dict?.ticket_uncompressed_size_in_bytes
                  ? formatNumber(
                      parseContractTicket?.ticket_input_data_dict
                        ?.ticket_uncompressed_size_in_bytes,
                    )
                  : parse(translate('common.na'))}
              </Box>
            </Grid>
