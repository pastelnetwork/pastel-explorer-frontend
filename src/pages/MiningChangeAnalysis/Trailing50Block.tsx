...
import { transferTrailing50BlockChartData } from './MiningChangeAnalysis.helpers';
...
const Trailing50Block = ({ data, isLoading }: TTrailing50BlockAverageBlocks) => {
  const renderContent = () => {
    if (isLoading) {
      return null;
    }
    ...
    const chartData = transferTrailing50BlockChartData(data || []);
    return (
      <LineChart
        chartName="miningChangeAnalysisTrailing50Block"
        dataX={chartData.dataX}
        dataY={chartData.dataY1}
        dataY1={chartData.dataY2}
      ...
      <SummaryStyles.CardContent className="header-wrapper">
        <SummaryStyles.ValueWrapper>
          <SummaryStyles.Typography variant="h6">
            {parse(translate('pages.miningChangeAnalysis.trailing50Block'))}
          </SummaryStyles.Typography>
        </SummaryStyles.ValueWrapper>
        <SummaryStyles.PercentageWrapper>
          <CascadeAndSenseStatisticsStyles.Percentage className="download-wrapper">
...
export default Trailing50Block;
