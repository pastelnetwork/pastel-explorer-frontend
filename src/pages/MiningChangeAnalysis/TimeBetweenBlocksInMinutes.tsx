...
import { transferTimeBetweenBlocksInMinutesChartData } from './MiningChangeAnalysis.helpers';
...
const TimeBetweenBlocksInMinutes = ({ data, isLoading }: TTrailing50BlockAverageBlocks) => {
  const renderContent = () => {
    if (isLoading) {
      return null;
    }
    ...
    const chartData = transferTimeBetweenBlocksInMinutesChartData(data || []);
    return (
      <LineChart
        chartName="timeBetweenBlocksInMinutes"
        dataX={chartData.dataX}
        dataY3={chartData.dataY}
        offset={chartData.offset}
        disableClick
        seriesName="pages.miningChangeAnalysis.timeBetweenBlocksInMinutes"
      />
    );
  };
      ...
      <SummaryStyles.CardContent className="header-wrapper">
        <SummaryStyles.ValueWrapper>
          <SummaryStyles.Typography variant="h6">
            {parse(translate('pages.miningChangeAnalysis.timeBetweenBlocksInMinutes'))}
...
export default TimeBetweenBlocksInMinutes;
