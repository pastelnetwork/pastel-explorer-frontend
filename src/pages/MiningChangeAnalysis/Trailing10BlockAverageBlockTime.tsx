...
import { transferTrailing10BlockAverageBlockTimeChartData } from './MiningChangeAnalysis.helpers';
...
const Trailing10BlockAverageBlockTime = ({ data, isLoading }: TTrailing50BlockAverageBlocks) => {
  const renderContent = () => {
    if (isLoading) {
      return null;
    }
    ...
    const chartData = transferTrailing10BlockAverageBlockTimeChartData(data || []);

    return (
      <LineChart
        chartName="trailing10BlockAverageBlockTime"
        dataX={chartData.dataX}
        dataY={chartData.dataY}
        offset={chartData.offset}
        disableClick
        seriesName="pages.miningChangeAnalysis.trailing10BlockAverageBlockTime"
      />
    );
  };
      ...
      <SummaryStyles.CardContent className="header-wrapper">
        <SummaryStyles.ValueWrapper>
          <SummaryStyles.Typography variant="h6">
            {parse(translate('pages.miningChangeAnalysis.trailing10BlockAverageBlockTime'))}
...
export default Trailing10BlockAverageBlockTime;
