...
import { totalOfSenseRequestsData } from './mockup';

const TotalOfSenseRequests: React.FC = () => {
  const [period, setPeriod] = useState<PeriodTypes>(periods[7][2]);
  const [chartData, setChartData] = useState(totalOfSenseRequestsData[0]);
  ...
  const handleDropdownChange = (
    ...
    if (event.target.value) {
      ...
      if (event.target.value === periods[7][0]) {
        setChartData(totalOfSenseRequestsData[1]);
      } else if (event.target.value === periods[7][3] || event.target.value === periods[7][4]) {
        setChartData(totalOfSenseRequestsData[2]);
      } else {
        setChartData(totalOfSenseRequestsData[0]);
      }
    }
  };
